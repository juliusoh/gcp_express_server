...
async function createTempCSVFileWithContent(content) {
  ...
  const tempFile = path.join(tempFolder, "vaccTimeSeries" + Date.now() + ".csv");
...
export async function whoVaccTimeSeries() {
  const bigQuery = new BigQuery(bigQueryCreds);
  let fileName;

  try {
    const response = await axios.get(`https://covid19.who.int/who-data/vaccination-data.csv`);
