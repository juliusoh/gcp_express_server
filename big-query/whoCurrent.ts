...
async function createTempCSVFileWithContent(content) {
  ...
  const tempFile = path.join(tempFolder, "whoCurrentData" + Date.now() + ".csv");
...
export async function whoCurrentData() {
  const bigQuery = new BigQuery(bigQueryCreds);
  let fileName;

  try {
    const response = await axios.get(`https://covid19.who.int/WHO-COVID-19-global-table-data.csv`);
