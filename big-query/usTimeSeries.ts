...
async function createTempCSVFileWithContent(content) {
  ...
  const tempFile = path.join(tempFolder, "usTimeSeries" + Date.now() + ".csv");
...
export async function getUSTimeSeries() {
  const bigQuery = new BigQuery(bigQueryCreds);
  let fileName;

  try {
    const response = await axios.get(
      `https://api.covidactnow.org/v2/country/US.timeseries.csv?apiKey=${process.env.API_KEY}`
    );
